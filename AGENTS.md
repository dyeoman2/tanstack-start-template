...
- **Always use migrations for schema changes**: `pnpm db:generate` then `pnpm db:migrate`.
- Never use `db:push` in production - migrations provide version control and safety.
...
- **Database workflow**: Edit schema → `pnpm db:generate` → `pnpm db:migrate` → commit migration files.
...
- ❌ Using `db:push` in production (use migrations for version control and safety)
...
- ✅ Database schema changes use migrations (`db:generate` + `db:migrate`)
