...
export const getAdminDashboardDataServerFn = createServerFn({ method: 'GET' }).handler(async () => {
  try {
    ...
    return {
      ...
      usersError: usersResult.status === 'rejected' ? getErrorMessage(usersResult.reason) : undefined,
      statsError: statsResult.status === 'rejected' ? getErrorMessage(statsResult.reason) : undefined,
    };
  } catch (error) {
