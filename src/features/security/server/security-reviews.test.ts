...
vi.mock('@convex/_generated/api', () => ({
  api: {
    securityReviews: {
      finalizeReviewRun: 'finalizeReviewRun',
    },
  },
}));
...
import { finalizeReviewRunServerFn } from './security-reviews';

describe('security review server functions', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    fetchAuthActionMock.mockResolvedValue({ id: 'review-run-1' });
  ...
  it('passes request context into review finalization actions', async () => {
    await finalizeReviewRunServerFn({
      data: {
        reviewRunId: 'review-run-1',
      },
    });

    expect(fetchAuthActionMock).toHaveBeenCalledWith('finalizeReviewRun', {
      reviewRunId: 'review-run-1',
      requestContext: {
        requestId: 'req-123',
        ipAddress: '203.0.113.9',
